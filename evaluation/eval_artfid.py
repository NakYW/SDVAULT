...
def compute_patch_simi(
    path_to_stylized, path_to_content, batch_size, device, num_workers=1
):
    """Computes the distance for the given paths.
    ...
    dataset_stylized = ImagePathDataset(
        stylized_image_paths, transforms=style_transforms
    )
    dataloader_stylized = torch.utils.data.DataLoader(
        dataset_stylized,
        batch_size=batch_size,
...
def compute_art_fid(
    ...
    if mode == "art_fid_inf":
        ...
        fid_value = compute_style_loss(
            path_to_stylized, path_to_style, batch_size, device, num_workers
        )
    else:
        fid_value = compute_gram_loss(
            path_to_stylized, path_to_style, batch_size, device, num_workers
        )

    print("Compute content distance...")
    cnt_value = compute_content_distance(
        path_to_stylized,
        path_to_content,
        batch_size,
        content_metric,
    ...
    gray_cnt_value = compute_content_distance(
        path_to_stylized,
        path_to_content,
        batch_size,
        content_metric,
...
def compute_cfsd(path_to_stylized, path_to_content, batch_size, device, num_workers=1):
    ...
    simi_val = compute_patch_simi(
        path_to_stylized, path_to_content, 1, device, num_workers
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Batch size for computing activations.",
    )
    parser.add_argument(
        "--num_workers",
        type=int,
    ...
    parser.add_argument(
        "--cnt", type=str, required=True, help="Path to content images."
    )
    parser.add_argument(
        "--tar", type=str, required=True, help="Path to stylized images."
    )
    args = parser.parse_args()

    start_style_time = time.time()

    # Time a first evaluation pass (not style transfer).
    compute_art_fid(
        args.tar,
        args.sty,
        args.cnt,
    ...
    cfsd = compute_cfsd(
        args.tar, args.cnt, args.batch_size, args.device, args.num_workers
    )

    end_eval_time = time.time()
    eval_elapsed_time = end_eval_time - start_eval_time

    print(f"Initial evaluation pass time: {style_elapsed_time:.4f}s")
